...
class GradientBoostedNN:
    def __init__(self, base_model, n_estimators=10, learning_rate=0.1):
        self.base_model = base_model  # The neural network class
        self.n_estimators = n_estimators
        self.learning_rate = learning_rate
        self.models = []  # List to store trained models
        self.alphas = []  # List to store learning rates for each model

    def fit(self, X, y, epochs=1000):
        residuals = y.copy()  # Initialize residuals
        for _ in range(self.n_estimators):
            # Initialize and train a new neural network
            model = self.base_model(inputSize=X.shape[1], hiddenLayerSize=32, outputSize=1, numHiddenLayers=2)
            model.train(X, residuals.reshape(-1, 1), epochs)
            predictions = model.predict(X).flatten()

            # Update residuals for the next model
            residuals -= self.learning_rate * predictions

            # Store the trained model and learning rate
    ...
    def predict(self, X):
        ...
        for model, alpha in zip(self.models, self.alphas):
            final_predictions += alpha * model.predict(X).flatten()
...
async def main():
    ...
    for ticker, df in stock_data.items():
        ...
        # Flatten the data to 2D for the neural network
        ...
        # Initialize and train the GradientBoostedNN
        gbnn = GradientBoostedNN(NeuralNetwork, n_estimators=10, learning_rate=0.1)
        gbnn.fit(X_train, y_train, epochs=1000)

        predictions = gbnn.predict(X_test)

        # Plotting
        fig = go.Figure()

        fig.add_trace(go.Scatter(y=y_test, mode='lines', name='Actual', line=dict(color='blue', width=2)))
        fig.add_trace(go.Scatter(y=predictions, mode='lines', name='Predicted', line=dict(color='orange', width=2)))

        fig.update_layout(
            title=f"GBNN Predictions vs Actual for {ticker}",
            xaxis_title="Index",
